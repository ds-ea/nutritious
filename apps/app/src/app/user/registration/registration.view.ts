...
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, Inject, OnDestroy, OnInit, TemplateRef, ViewChild, ViewContainerRef } from '@angular/core';
							...
							<mat-card-content>
								<form class="form-vertical" [formGroup]="registrationForm" (submit)="studyCheck()" autocomplete="off">
									...
									<div class="content-actions">
										...
										>{{ 'USER.AUTH.REGISTER_BTN' | translate }}
										</button>
									</div>
								</form>
							</mat-card-content>
						...
						<ng-template [ngSwitchCase]="SignupStep.SignUp">
							<mat-card-content>
								<form class="form-vertical" [formGroup]="participantForm" (submit)="signup()" autocomplete="off">
									...
									<h2 class="study-title">{{ signupCheckMeta?.study!.name }}</h2>
									@if (signupCheckMeta?.instructions) {
										<markdown [data]="signupCheckMeta?.instructions || ''"></markdown>
									}

									<ion-note class="explain">{{ 'USER.SIGNUP.CODE_VALID_CONFIRM_PARTICIPATION' | translate }}</ion-note>

									<mat-form-field>
										<mat-label>{{ 'USER.SIGNUP.PARTICIPANT_BADGE_LBL' | translate }}</mat-label>
										<input matInput [placeholder]="'USER.SIGNUP.PARTICIPANT_BADGE_LBL' | translate"
											   formControlName="participant"
										/>
										<mat-hint>{{ 'USER.SIGNUP.PARTICIPANT_BADGE_HINT' | translate }}</mat-hint>
									</mat-form-field>

									<div class="content-actions extra-space">
										<button type="submit" mat-flat-button color="primary"
												[disabled]="!registrationForm.valid"
										>{{ 'USER.SIGNUP.CONFIRM_AND_SUBMIT_SIGNUP_BTN' | translate }}
										</button>
									</div>
								</form>
							</mat-card-content>
						...
						<ng-template [ngSwitchCase]="SignupStep.LogIn">
							<mat-card-content>
								<form class="form-vertical" [formGroup]="loginForm" (submit)="login()" autocomplete="off">
									...
									<p [innerHtml]="'USER.SIGNUP.CREDENTIALS_HEADER' | translate: {name:signupMeta?.study!.name} "></p>
									<ul class="credentials" *ngIf="credentials; else noCredentials">
										<li>{{ 'USER.SIGNUP.CREDS_LOGIN_LBL' | translate }}: <strong>{{ credentials.login }}</strong></li>
										<li>{{ 'USER.SIGNUP.CREDS_PASS_LBL' | translate }}: <strong>{{ credentials.password }}</strong></li>
										<li *ngIf="signupMeta?.badge">{{ 'USER.SIGNUP.PARTICIPANT_BADGE_LBL' | translate }}: <strong>{{ signupMeta?.badge }}</strong></li>
									</ul>
...
export class RegistrationView implements OnInit, OnDestroy{
	...
	public signupCheckMeta?:SignupCheckResponse;
	public signupMeta?:SignupResponse;
	public credentials?:ParticipantCredentials;
	...
	public ngOnDestroy():void{
		if( 'nutriManualQRCode' in window )
			delete window['nutriManualQRCode'];
	...
	public async startScan(){
		...
		if( result.hasContent ){
			this.processCode( result.content );
			this.afterScan();
			this.studyCheck();
		}
	}
	...
	public async studyCheck(){
		...
		this.core.signupCheck( data.key, data.code, this.apiURLBase )
	...
	private processDomain( domain:string ):string | undefined{
		domain = domain.trim();
		const urlRaw = domain.includes( '://' ) ? domain : `https://${ domain }`;

		try{
			const url = new URL( urlRaw );
			return url ? urlRaw : undefined;

		}catch( e ){
			this.registrationForm.get( 'domain' )?.setErrors( { domain: 'invalid domain' } );
			return undefined;
		}
	}
	...
	public async signup(){
		...
		if( !this.apiURLBase && domain.length ){
			this.apiURLBase = this.processDomain( domain );
			if( !this.apiURLBase )
				return;
		}
		...
		this.core.signup( data.key, data.code, participantData.participant, this.apiURLBase )
			.pipe(
			...
			.subscribe( result => {
				...
				if( !result?.study || !result.credentials ){
					this.toaster.create( {
						color: 'danger',
						message: this.translate.instant( 'USER.SIGNUP.NO_CREDENTIALS_ERR' ),
						duration: 3000,
					} ).then( toast => toast.present() );
				}else{
					this.signupMeta = result;
					this.credentials = result.credentials;

					this.loginForm.patchValue( { login: result.credentials.login } );
	...
	public async login(){
		...
		const domain = this.loginForm.get( 'domain' )?.value?.trim();
		if( !this.apiURLBase && domain?.length ){
			this.apiURLBase = this.processDomain( domain );
			if( !this.apiURLBase ){
				return;
	...
	private processCode( content:string ){

		if( content.indexOf( '█' ) !== -1 ){ // new nutritious processing
			...
			if( version === '0x' ){ // decrypt when necessary
				...
				rawParts.forEach( ( value, key, parts ) =>
					parts[key] = xorEncryptDecrypt( value, qrKey!, qrSalt ) );
			}

			const [ key, code, shortenedDomain, ...parts ] = rawParts;
			const url = 'http' + ( shortenedDomain.charAt( 0 ) == '1' ? 's' : '' ) + '://' + shortenedDomain.slice( 1 );
			this.apiURLBase = url;

			const domain = new URL( url )?.hostname;

			const data = { key, code, domain };
			this.registrationForm.patchValue( data );
			this.registrationForm.markAllAsTouched();


			for( const k of Object.keys( data ) ){
				const ctrl = this.registrationForm.get( k );
				if( ctrl ){
					ctrl.markAsDirty();
					ctrl.markAsTouched();
				}
			...
			const [ key, code ] = content?.split( ':', 2 ) || [];
